<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			const payload = result.data?.createRace;
			if (payload?.__typename === 'Race') {
				toast.success('Race created');
				goto(`/database/races/${payload.id}`);
...
<div class="content-pad db-page">
	<a href="/database/races" class="inline-flex items-center gap-2 text-xs text-text-muted uppercase tracking-wider transition-colors hover:text-accent-amber">
		<ArrowLeft class="h-3.5 w-3.5" /><span>Back to Races</span>
	</a>

	<div class="border border-border-dim bg-hull px-3 py-3">
		<span class="machine-text text-[9px] text-text-muted/60 block mb-1">RCE // NEW ENTRY</span>
		<h1 class="title-display text-lg text-accent-amber text-glow-amber">NEW RACE</h1>
	</div>
	...
	<Panel class="max-w-2xl">
		<form onsubmit={handleSubmit} class="stack-space">
			<div>
				...
				<input id="name" bind:value={name} class="w-full bg-void border border-border-dim px-2.5 py-2 text-text-primary placeholder:text-text-muted/50 focus:border-accent-amber focus:outline-none" placeholder="Race name" />
			</div>
			<div>
