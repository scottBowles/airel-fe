<script lang="ts">
	...
	let { RaceList } = $derived(data);
</script>

<div class="space-y-6">
	<div class="border-industrial-dim flex items-center justify-between border-b pb-4">
		<div>
			<div class="mb-1 font-mono text-xs text-slate-500">DATABASE / SPECIES</div>
			<h2 class="text-3xl font-bold tracking-widest text-slate-100 uppercase">Races</h2>
		</div>
		<div class="text-industrial-amber font-mono text-xs">
			Count: {$RaceList.data?.races?.edges.length || 0}
		</div>
	</div>
	...
	<div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
		{#if $RaceList.data?.races?.edges}
			{#each $RaceList.data.races.edges as edge (edge.node.id)}
				<a
					href={resolve(`/database/races/${edge.node.id}`)}
