<script lang="ts">
	...
	let raceEdges = $derived(sortEdgesByUpdatedDesc($RaceList.data?.races?.edges));
</script>
...
<div class="db-page">
	...
	<div class="db-grid">
		{#if raceEdges.length}
			{#each raceEdges as edge (edge.node.id)}
				<a href={resolve(`/database/races/${edge.node.id}`)} class="group db-card">
