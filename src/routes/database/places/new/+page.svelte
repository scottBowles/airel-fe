...
<div class="content-pad db-page">
	...
	<div class="border border-border-dim bg-hull px-3 py-3">
		<span class="machine-text text-[9px] text-text-muted/60 block mb-1">LOC // NEW ENTRY</span>
	...
	<Panel class="max-w-2xl">
		<form onsubmit={handleSubmit} class="stack-space">
			<div>
				...
				<input id="name" bind:value={name} class="w-full bg-void border border-border-dim px-2.5 py-2 text-text-primary placeholder:text-text-muted/50 focus:border-accent-amber focus:outline-none" placeholder="Place name" />
			</div>
			<div>
