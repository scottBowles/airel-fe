<script lang="ts">
	...
	let itemEdges = $derived(sortEdgesByUpdatedDesc($ItemList.data?.items?.edges));
</script>
...
<div class="db-page">
	...
	<div class="db-grid">
		{#if itemEdges.length}
			{#each itemEdges as edge (edge.node.id)}
				<a href={resolve(`/database/items/${edge.node.id}`)} class="group db-card">
