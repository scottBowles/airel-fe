<script lang="ts">
	...
	let { ArtifactList } = $derived(data);
</script>

<div class="space-y-6">
	<div class="border-industrial-dim flex items-center justify-between border-b pb-4">
		<div>
			<div class="mb-1 font-mono text-xs text-slate-500">DATABASE / ARTIFACTS</div>
			<h2 class="text-3xl font-bold tracking-widest text-slate-100 uppercase">Artifacts</h2>
		</div>
		<div class="text-industrial-amber font-mono text-xs">
			Count: {$ArtifactList.data?.artifacts?.edges.length || 0}
		</div>
	</div>
	...
	<div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
		{#if $ArtifactList.data?.artifacts?.edges}
			{#each $ArtifactList.data.artifacts.edges as edge (edge.node.id)}
				<a
					href={resolve(`/database/artifacts/${edge.node.id}`)}
				...
				>
					<div class="mb-3 flex items-center gap-3">
						...
						{:else}
							<ScifiPlaceholder type="artifact" />
						{/if}
						<div class="min-w-0">
							<h3
